<script lang="ts">
	...
	urbanMenu.subscribe(value => urban = value);
	ruralMenu.subscribe(value => rural = value);

	// Flatten the menus array to get all excluded labels
	const excludedLabels = [...urban, ...rural];
		...
		{#if $page.url.pathname !== '/dashboard'}
			<div class="flex items-center min-w-[40vw] ml-6">
				{#each breadcrumbs as breadcrumb, index}
					...
					<a
						href={breadcrumb.url}
						class="capitalize text-neutral-500 hover:underline font-semibold hover:text-theme"
					>
						{breadcrumb.label}
					</a>
				{/each}
