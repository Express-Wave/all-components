...
<header
	...
	class="h-[100vh] max-h-[100vh] flex flex-col min-h-full bg-theme hide-scrollbar overflow-hidden border-r border-neutral-200 transition-all duration-500 ease-in-out text-mono-100"
	style={`width: ${closed ? '4rem' : `${width}rem`};`}
>
	<button
		class={`flex cursor-pointer min-h-[${height}] max-h-12 items-center border-b px-6 border-neutral-500`}
		on:click={toggleSidebar}
	>
		{#if closed}
			<MenuIcon currentColor="white"/>
		{:else}
	...
	<a
		...
		href="/"
		role="menuitem"
		class="block -mt-1 flex flex-col border-t py-5 px-6 border-neutral-500"
		style="position: sticky; bottom: 0;"
	>
		<div
			class="cursor-pointer flex items-center outline-none focus-visible:ring-1-muted focus-visible:z-10 group -ml-[0.2rem] font-normal border-neutral-500"
		>
			<div class="transition truncate text-sm group-hover min-w-fit">
				<LogoutIcon currentColor="white"/>
			</div>
			<span class="transition capitalize truncate text-md text-white  w-full pl-4">
				Extron cloud help center
			</span>
		</div>
	</a>
</header>