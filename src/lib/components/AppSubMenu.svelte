...
<script lang="ts">
  ...
  function updateMenu(pathname: string) {
    ...
    // Extract the last segment of the current pathname
  ...
  // Condition to show the menu only when the path includes 'q'
  $: shouldShowMenu = (currentMenu.length > 0) && ($page.url.pathname.includes('q') || $page.url.pathname.includes('Q'));
</script>
      ...
      {#each currentMenu as item}
        <li class="me-2">
          <a
            href={`/forms/${isUrbanPath ? 'urban' : 'rural'}/${year}/${quarter}/${item}`}
            class={`capitalize inline-block p-4 border-b-2 rounded-t-lg
              ${isActive(item) ? 'text-[var(--clr-theme-color)] font-semibold border-[var(--clr-theme-color)] dark:text-[var(--clr-theme-color)] dark:border-[var(--clr-theme-color)]' : 'text-gray-500 border-transparent hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300'}
              ${item === 'disabled' ? 'text-gray-400 cursor-not-allowed dark:text-gray-500' : ''}
            `}
            aria-current={isActive(item) ? 'page' : undefined}
          >
